...
| `CLAUDE_CODE_OAUTH_TOKEN`                  | Long-lived token from `claude setup-token`. When set, it is used as-is: the Keychain / credentials file is not read and no OAuth refresh, 401 recovery, or 429 token rotation is attempted. Account switching is disabled while set. Read once at startup.                        | unset                                                              |
