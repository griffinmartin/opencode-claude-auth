...
| `OPENCODE_CLAUDE_AUTH_TOOL_REPAIR`  | Strategy for reconciling `tool_use`/`tool_result` adjacency broken by OpenCode auto-compaction. `placeholder` synthesizes a paired result for orphaned `tool_use` blocks (lossless, preserves `thinking` blocks); `drop` removes orphaned blocks (omitting whole thinking turns). See issues [#212](https://github.com/griffinmartin/opencode-claude-auth/issues/212)/[#261](https://github.com/griffinmartin/opencode-claude-auth/issues/261). | `placeholder`                                                      |
