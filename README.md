...
| `OPENCODE_CLAUDE_AUTH_REFRESH_WAIT_MS`     | Max ms a single request waits through a transient token-refresh rate-limit (429) before returning a retryable error instead of a hard "run `claude`". The request returns as soon as the cooldown clears or a sibling instance/CLI writes a fresh token. | `45000`                                                            |
