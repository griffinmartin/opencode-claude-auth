...
| `ANTHROPIC_BASE_URL`                | Destination host for Anthropic API requests. OAuth headers are still attached by the plugin as normal; only the host changes. Useful for routing through a local optimization/logging proxy (e.g. Headroom). | `https://api.anthropic.com/v1`                                     |
